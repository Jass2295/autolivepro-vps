...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>AutoLive Pro — Files</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  :root{--bg:#08080f;--card:#10101c;--border:rgba(255,45,95,0.18);--accent:#ff2d5f;--text:#e8e8f0;--muted:#6b6b88;--green:#22c55e;--red:#ef4444;--yellow:#f59e0b}
  ...
  header{display:flex;align-items:center;justify-content:space-between;padding:14px 24px;border-bottom:1px solid var(--border);background:rgba(10,10,20,0.97);position:sticky;top:0;z-index:10}
  .logo-text{font-size:15px;font-weight:700}.logo-text span{color:var(--accent)}
  .logo-sub{font-size:10px;color:var(--muted);letter-spacing:2px;text-transform:uppercase}
  .nav{display:flex;gap:10px}
  .nav a{text-decoration:none;color:var(--muted);font-size:13px;padding:6px 14px;border:1px solid var(--border);border-radius:8px;transition:all .2s}
  .nav a:hover{color:var(--text);border-color:rgba(255,255,255,0.2)}
  #pin-screen{flex:1;display:flex;align-items:center;justify-content:center}
  .pin-box{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:40px;width:340px;text-align:center}
  .pin-icon{font-size:36px;margin-bottom:14px}
  .pin-box h3{font-size:18px;margin-bottom:6px}
  .pin-box p{font-size:13px;color:var(--muted);margin-bottom:22px}
  .pin-input{width:100%;background:#0a0a16;border:1px solid var(--border);color:var(--text);padding:12px 16px;border-radius:10px;font-size:16px;text-align:center;letter-spacing:4px;outline:none;transition:border-color .2s;margin-bottom:12px}
  .pin-input:focus{border-color:var(--accent)}
  .pin-btn{width:100%;background:var(--accent);color:#fff;border:none;padding:12px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;transition:opacity .2s}
  .pin-btn:hover{opacity:.85}
  .pin-err{color:#ef4444;font-size:12px;margin-top:8px;display:none}
  #fm-screen{flex:1;display:none;flex-direction:column;overflow:hidden}
  .toolbar{display:flex;align-items:center;gap:10px;padding:12px 20px;background:#0a0a16;border-bottom:1px solid var(--border);flex-wrap:wrap}
  .breadcrumb{flex:1;font-size:13px;color:var(--muted);font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .breadcrumb span{color:var(--text)}
  .tbtn{background:rgba(255,45,95,0.1);color:var(--accent);border:1px solid rgba(255,45,95,0.3);border-radius:8px;padding:7px 14px;font-size:12px;font-weight:600;cursor:pointer;transition:all .2s;white-space:nowrap}
  .tbtn:hover{background:rgba(255,45,95,0.22)}
  .tbtn.green{background:rgba(34,197,94,0.1);color:var(--green);border-color:rgba(34,197,94,0.3)}
  .tbtn.green:hover{background:rgba(34,197,94,0.2)}
  .tbtn.red{background:rgba(239,68,68,0.1);color:var(--red);border-color:rgba(239,68,68,0.3)}
  ...
  .file-item{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:16px 14px;cursor:pointer;transition:all .2s;position:relative;user-select:none}
  .file-item:hover{border-color:rgba(255,45,95,0.4);background:#141420}
  .file-item.selected{border-color:var(--accent);background:rgba(255,45,95,0.08)}
  .file-icon{font-size:28px;margin-bottom:8px;text-align:center}
  .file-name{font-size:12px;font-weight:600;word-break:break-all;text-align:center;margin-bottom:4px}
  .file-meta{font-size:10px;color:var(--muted);text-align:center}
  .file-check{position:absolute;top:8px;right:8px;width:16px;height:16px;border-radius:50%;border:2px solid var(--border);background:transparent;transition:all .2s}
  ...
  .status-bar{padding:8px 20px;background:#0a0a16;border-top:1px solid var(--border);font-size:12px;color:var(--muted);display:flex;gap:16px}
  .status-bar b{color:var(--text)}
  .upload-zone{border:2px dashed var(--border);border-radius:12px;padding:30px;text-align:center;color:var(--muted);font-size:13px;margin-bottom:16px;transition:all .2s;cursor:pointer}
  .upload-zone.drag{border-color:var(--accent);background:rgba(255,45,95,0.06)}
  #upload-input{display:none}
  .progress-bar{height:4px;background:rgba(255,255,255,0.06);border-radius:4px;margin-top:8px;overflow:hidden;display:none}
  .progress-fill{height:100%;background:linear-gradient(90deg,var(--accent),#ff6b8a);width:0%;transition:width .3s;border-radius:4px}
  .toast{position:fixed;bottom:20px;right:20px;background:var(--card);border:1px solid var(--border);border-radius:10px;padding:12px 18px;font-size:13px;z-index:999;display:none;animation:fadein .2s}
...
<body>
<header>
  <div><div class="logo-text">AutoLive <span>Pro</span> — Files</div><div class="logo-sub">File Manager</div></div>
  <div class="nav">
    <a href="/terminal">⌨ Console</a>
    <a href="/">← Dashboard</a>
  </div>
</header>
...
<div id="pin-screen">
  <div class="pin-box">
    <div class="pin-icon">📁</div>
    <h3>File Manager</h3>
    <p>Enter Console PIN to access files</p>
    <input id="pin-input" class="pin-input" type="password" placeholder="••••••" maxlength="32" onkeydown="if(event.key==='Enter')doPin()"/>
    <button class="pin-btn" onclick="doPin()">Open File Manager</button>
    <div id="pin-err" class="pin-err">Wrong PIN — try again</div>
  </div>
</div>
...
<div id="fm-screen">
  <div class="toolbar">
    <div class="breadcrumb" id="breadcrumb">/ home / ubuntu</div>
    <button class="tbtn" onclick="goUp()">⬆ Up</button>
    <button class="tbtn" onclick="newFolder()">📁 New Folder</button>
    <label class="tbtn green" style="cursor:pointer">⬆ Upload<input type="file" id="upload-input" multiple onchange="uploadFiles(this.files)"></label>
    <button class="tbtn red" onclick="deleteSelected()" id="del-btn" style="display:none">🗑 Delete</button>
    <button class="tbtn" onclick="loadDir(currentPath)">↺ Refresh</button>
  </div>
  <div class="file-list">
...
async function doPin() {
  ...
  if (r.ok) {
    _pin = v;
    document.getElementById('pin-screen').style.display = 'none';
    document.getElementById('fm-screen').style.display = 'flex';
    loadDir(currentPath);
  } else {
    document.getElementById('pin-err').style.display = 'block';
  }
...
function fileIcon(item) {
  ...
  if (['js','ts','json','py','sh','html','css'].includes(ext)) return '📄';
...
async function loadDir(p) {
  ...
  const r = await fetch('/files/list', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ pin: _pin, path: p })
  ...
  document.getElementById('status-count').textContent = r.items.length + ' items';
  grid.innerHTML = r.items.map(item => {
    const key = item.path;
    return \`<div class="file-item" data-path="\${key}" data-dir="\${item.dir}" ondblclick="itemDblClick('\${key}',\${item.dir})" onclick="itemClick(event,'\${key}')">
      <div class="file-check"></div>
      <div class="file-icon">\${fileIcon(item)}</div>
      <div class="file-name">\${item.name}</div>
      <div class="file-meta">\${item.dir ? 'Folder' : fmtSize(item.size)}</div>
    </div>\`;
  }).join('');
}

function itemDblClick(p, isDir) {
  if (isDir) loadDir(p);
  else downloadFile(p);
}

function itemClick(e, p) {
  const el = e.currentTarget;
  if (selected.has(p)) { selected.delete(p); el.classList.remove('selected'); }
  else { selected.add(p); el.classList.add('selected'); }
  updateSelUI();
}

function updateSelUI() {
  const n = selected.size;
  document.getElementById('status-sel').textContent = n ? n + ' selected' : '';
  document.getElementById('del-btn').style.display = n ? 'inline-block' : 'none';
}
...
async function newFolder() {
  ...
  if (!name) return;
  const r = await fetch('/files/mkdir', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ pin: _pin, path: currentPath + '/' + name })
  }).then(r=>r.json());
  if (r.ok) { showToast('Folder created'); loadDir(currentPath); }
...
function downloadFile(p) {
  window.open('/files/download?pin=' + encodeURIComponent(_pin) + '&path=' + encodeURIComponent(p));
}
...
document.getElementById('pin-input').addEventListener('keydown', e => { if(e.key==='Enter') doPin(); });
</script>
</body>
</html>`;
}
...
app.get("/files", (_req, res) => res.send(fileManagerHTML()));

// ─── File Manager API ─────────────────────────────────────────────────────────
function pinOk(pin) { return pin && safeEqual(String(pin), String(CONSOLE_PIN)); }
function safePath(p) {
  const resolved = path.resolve(p || "/home/ubuntu");
  return resolved;
}

app.post("/files/list", (req, res) => {
  const { pin, path: p } = req.body || {};
  if (!pinOk(pin)) return res.status(401).json({ error: "Unauthorized" });
...
app.post("/files/upload", upload.single("file"), (req, res) => {
  ...
  const dest = req.body && req.body.path;
  if (!pinOk(pin)) { fs.unlinkSync(req.file.path); return res.status(401).json({ error: "Unauthorized" }); }
  try {
