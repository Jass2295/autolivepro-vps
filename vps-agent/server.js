...
// ─── Notify data server when stream stops ─────────────────────────────────────
...
// ─── FFmpeg (COPY MODE — no transcode, max performance) ───────────────────────
...
function buildFfmpegArgs(cfg, videoInputs) {
  const rtmpUrl = buildRtmpUrl(cfg.platform, cfg.stream_key);
  const args = [];

  if (videoInputs.length > 1) {
    // Playlist mode — concat with copy
  ...
  // COPY MODE — CPU minimal, no re-encoding
  args.push(
    "-c", "copy",         // copy video + audio — no transcode
    "-f", "flv",
...
function startFfmpeg(slotId, cfg, restartCount = 0) {
  ...
  const args = buildFfmpegArgs(cfg, videoInputs);
  ...
  proc.stderr.on("data", chunk => {
    // Only log errors, not normal ffmpeg progress
  ...
  proc.on("exit", (code, signal) => {
    ...
    if (!manual && code !== 0 && restartCount < 10) {
      const delay = Math.min(5000 * Math.pow(1.5, restartCount), 60000);
...
app.post("/stream/start", auth, (req, res) => {
  ...
  console.log(`[${slot_id}] started (${isPlaylist ? `playlist ${video_urls.length} videos` : "single"}) — copy mode`);
  res.json({ ok: true, slot_id, mode: "copy" });
});
...
app.post("/files/mkdir", auth, (req, res) => {
  const { pin, path: p } = req.body || {};
  if (!pinOk(pin)) return res.status(401).json({ error: "Invalid PIN" });
...
app.post("/files/upload", auth, upload.single("file"), (req, res) => {
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`\n🚀  AutoLive Pro VPS Agent v2 — port ${PORT}  (max: ${MAX_STREAMS} streams, COPY MODE)\n`);
});
